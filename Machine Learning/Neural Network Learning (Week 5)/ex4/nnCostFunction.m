...
for i = 1:size(X),	%set y(k) to be a 
	yk(i,y(i)) = 1;
end
...
for i = 1:m,
	...
	J += (1/m) * (-yk(i,:) * log(a3) - (1 - yk(i,:)) * log(1 - a3));
...
%regularize standard cost function
T1 = Theta1(:,2:end);
T2 = Theta2(:,2:end);
...
Theta1_grad = (1/m) * Theta1_grad + (lambda/m) * T1Reg;
Theta2_grad = (1/m) * Theta2_grad + (lambda/m) * T2Reg;
